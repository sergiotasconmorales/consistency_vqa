...
def main():
    ...
    for q_type in ['inside', 'whole']: # for questions about regions and about whole image
    ...
    for img in images_one_grade:
        ...
        counts_dict = counts_dict = find_and_count_answers(img, 'whole', qa_pairs_test)
    ...
    for img in q3_wrong_images_zero_grade: # for ever image
        ...
        if ans_whole_zero == 'no':
            # check that all inside questions about this image are negative
            ...
            if len(list(set(ans_inside))) == 0:
                print(' Image', img, 'has partial Q3 inconsistencies (whole is right but no inside are available to test further)')
